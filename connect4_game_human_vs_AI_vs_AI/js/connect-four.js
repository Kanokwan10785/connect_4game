...
Game.prototype.act = function(e) {
    ...
    // Player 1 (Human) round
    if (that.round === 0) that.place(element.cellIndex);
    
    // Player 2 (Computer) round
    else if (that.round === 1) 
        that.generateComputerDecision(that.round);
        setTimeout(function() {
            if (that.round == 2)that.generateComputerDecision(that.round);
        },100);
        ...
        for (var y = that.rows - 1; y >= 0; y--) {
            if (document.getElementById('game_board').rows[y].cells[column].className == 'empty') {
                if (that.round === 1) {
                    ...
                    document.getElementById('game_board').rows[y].cells[column].className = 'coin third-coin';
                } 
                break;
            }
        }
...
Game.prototype.switchRound = function(round) {
    // 0 Human, 1 Computer, 2 Human (third player)
    if (round === 0) {
        return 1;
    } else if (round === 1) {
        return 2;
...
Game.prototype.updateStatus = function() {
    ...
    if (that.board.score() == -that.score) {
        that.status = 1;
        that.markWin();
        statusMessage = "Player 1 (Human) has won!";
    }
    ...
    if (that.board.score() == that.score) {
        that.status = 2;
        that.markWin();
        statusMessage = "Player 2 (Computer) has won!";
    }

    // Third player won
    if (that.board.score() == that.score * 2) {
        that.status = 3;
        that.markWin();
        statusMessage = "Player 3 has won!";
    }
    ...
    if (that.status == 0) {
        ...
        html.innerHTML = "Status: Running";
    } else if (that.status == 1 || that.status == 2 || that.status == 3) {
        html.className = "status-won";
        html.innerHTML = "Status: " + statusMessage;
    } else {
